...
create table if not exists documents (
  ...
  ciphertext bytea,
  enc smallint not null default 0,
  updated_at timestamptz default now()
);
...
alter table document_revisions add column if not exists ciphertext bytea;
alter table document_revisions add column if not exists enc smallint not null default 0;
...
drop policy if exists "user_vault owner select" on user_vault;
create policy "user_vault owner select" on user_vault
  for select using (auth.uid() = user_id);
revoke all on user_vault from public, anon, authenticated;
grant select on user_vault to authenticated;
