-- Trashing essays/ or drafts/ used to mint a twin at the workspace root
-- on the next editor boot: ensure_default_workspace only looked for those
-- folders at parent_id is null. Look anywhere in the tree (Trash included)
-- and only insert when no folder with that name exists.
